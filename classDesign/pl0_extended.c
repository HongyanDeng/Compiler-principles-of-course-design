			...
			else
			{
				...
				else
				{
					if (sym == ifsym)
					{
						...
						cx1 = cx;
						gendo(jpc, 0, 0); // 条件为假时跳转

						statementdo(fsys, ptx, lev); // THEN 分支

						if (sym == elsesym)
						{
							getsymdo;
							cx2 = cx;
							gendo(jmp, 0, 0);       // 跳过 else 分支
							code[cx1].a = cx;       // 填写 jpc 跳转目标

							statementdo(fsys, ptx, lev); // ELSE 分支
							code[cx2].a = cx;       // 填写 jmp 跳转目标
						}
						else
						{
							code[cx1].a = cx; // 没有 else，跳过 then 即可
						}
					}
